var HexaToken = artifacts.require("./HexaToken.sol");

contract("HexaToken", function (accounts) {
  let tokenInstance;

  it("initializes the contract with the correct values", function () {
    return HexaToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
        return tokenInstance.name();
      })
      .then(function (name) {
        assert.equal(name, "HexaToken", "has the correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "HEXA", "has the correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "HEXA Token v1.0", "has the correct standard");
      });
  });
